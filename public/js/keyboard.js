 ...
 *   --app-h  目前真正可見的高度（#app 拿它當滿版高度；沒有鍵盤時不設定）
 *   --kb     鍵盤遮住的高度（沒有鍵盤時為 0px，給 position:fixed 的元素讓開）
 * #app 一縮，側欄抽屜與各畫面都跟著縮，聊天輸入框自然浮到鍵盤上面。
 * 輸入框聚焦後再分次把它捲進可見範圍，舊版瀏覽器沒有 visualViewport 時
 * 這層也還能保底。
 *
 * 【使用】
...
(function (w, d) {
  ...
  function visibleHeight() {
    if (usable()) return Math.round(vv.height);
    return w.innerHeight || root.clientHeight || 0;
  }

  /* 可見區的上下緣；getBoundingClientRect 是版面座標，可見區則會被
     visualViewport 往下推 offsetTop，兩者要換算到同一套座標才比得準。 */
  function visibleTop() {
    return usable() ? Math.round(vv.offsetTop) : 0;
  }
  ...
  function keepVisible(el) {
    ...
    var delta = Math.round(box.bottom - bottom);
